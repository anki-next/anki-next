...
"
    ...
    contents.push("export type AnkiService = {".to_owned());
    for service in services {
        let (name, camel) = short_service_name(&service.name);
        contents.push(format!("  {camel}: IBackend{name}Client&I{name}Client,"));
    }
...
"
    ...
    let unions = services
        .iter()
        .map(|svc| {
            let (_, camel) = short_service_name(&svc.name);
            format!("IsEqual<keyof AnkiService['{camel}'], keyof ((typeof METHODS)['{camel}'])>")
        })
        .collect::<Vec<_>>()
        .join("&");
